...
export function QuestionBankGlossary() {
  ...
  return (
    <div className="neon-card rounded-2xl p-4 md:p-5">
      ...
      <p className="mt-4 text-sm text-zinc-500">
        ...
        {category !== "全部" || query.trim() ? `（筛选自 ${QUESTION_BANK_GLOSSARY.length}）` : ""}
      </p>
      ...
      {filtered.length === 0 ? (
        <div className="mt-6 rounded-xl border border-dashed border-zinc-700 bg-zinc-950/50 px-4 py-10 text-center text-sm text-zinc-400">
          没有匹配术语，试试换个关键词。
        </div>
      ) : null}

      <p className="mt-6 text-xs leading-5 text-zinc-500">
        术语覆盖 AI 产品面试常见考点（检索、Agent、评测、产品方法与工具）。后续可按你的题库 Tags 继续扩充。
      </p>
    </div>
  );
}
