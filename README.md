...
* **大模型调度**（Vercel AI SDK + 统一路由层 `src/lib/llm.ts`）：
  ...
  * **Gemini 3.5 Flash** — 公司研究、JD 解码、定位分析等深度分析模块
...
* **大语言模型：** Qwen3.7-Max、Claude Sonnet 4.6、DeepSeek V4-Pro/Flash、Gemini 3.5 Flash（见下方「大模型路由」）
...
| `pro` | Gemini 3.5 Flash | Google | OpenAI Compatible |

> 说明：Gemini 分析统一使用 **`pro`** 档位（`gemini-3.5-flash`）。本地若曾保存已废弃的 `deep`，会自动迁移为 `pro`。
...
| 公司研究 | Gemini 3.5 Flash | `/api/research/analyze` |
| JD 解码 / 7 天备战计划 | Gemini 3.5 Flash | `/api/decode/analyze`、`/api/decode/plan` |
| 求职定位 / 提振包 / 故事 AI 优化 | Gemini 3.5 Flash | `/api/positioning/analyze`、`/api/hype/generate` |
| LinkedIn / 档案优化 | Gemini 3.5 Flash | `/api/linkedin/optimize` |
...
| Gemini 3.5 Flash | `https://generativelanguage.googleapis.com/v1beta/openai/` | `gemini-3.5-flash` |
